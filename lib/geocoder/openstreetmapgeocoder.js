        ...
        if (err) {
            ...
            if (result instanceof Array) {
              for (var i = 0; i < result.length; i++) {
                var formatedResult = _this._formatResult(result[i]);
                if (result[i].importance) {
                  if (! formatedResult.extra) {
                      formatedResult.extra = {};
                  }
                  formatedResult.extra.score=result[i].importance;
                }
                results.push(formatedResult);
              }
            } else {
              var formatedResult = _this._formatResult(result);
              if (result.importance) {
                if (! formatedResult.extra) {
                    formatedResult.extra = {};
                }
                formatedResult.extra.score=result.importance;
              }
              results.push(formatedResult);
            }
